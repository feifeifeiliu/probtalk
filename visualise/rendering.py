...
class RenderTool():
        ...
        for i_frame in tqdm(range(num_frames)):
            ...
            for i in range(len(v_list)):
                if face:
                    ...
                    img = render_mesh_helper(Mesh(v_list[i][i_frame], self.template_mesh.f), center, camera='o',r=r, y=0.7, rotation_times=rotation_times)
                ...
                # if mask is not None:
                #     if mask[i_frame] == 1:
                #         img = add_image_text(img, 'Ground Truth', (0,0,200))
                #     else:
                #         img = add_image_text(img, 'Generated Motion', (0,100,0))
                cur_img.append(img)

            final_img = cur_img[0].astype(np.uint8)
            # final_img = add_image_text(final_img, 'time'+str(i_frame/30), w=width, h=height).astype(np.uint8)
    ...
    def _render_sequences_helper(self, video_fname, cur_wav_file, v_list, stand, face, whole_body, transcript, mask):
