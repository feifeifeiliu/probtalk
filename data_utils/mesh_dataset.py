...
from data_utils.consts import speaker_id, get_speaker_id
import random
from data_utils.rotation_conversion import axis_angle_to_matrix, matrix_to_rotation_6d
from data_utils.foundation_models import get_textfeat, get_textfeat_bin


def renaming_suffix(file, suffix='.txt'):
...
# with open('data_utils/speaker_id.pickle', 'rb') as f:
...
class SmplxDataset():
    ...
    def __init__(self,
        ...
        if load_mode == 'pickle':
            ...
            if self.load_all:
                self._load_them_all(fm_dict, motion_fn)
    ...
    def _load_them_all(self, fm_dict, motion_fn):
        self.loaded_data = {}
        self.complete_data = []
        f = open(motion_fn, 'rb+')
        data = pickle.load(f)
        ...
        self.complete_data = np.array(self.complete_data, dtype='f')

        if fm_dict['aud_m'] is not None or self.config.Model.encoder_choice is not None:
            ...
            self.audio_feat = np.zeros([full_body.shape[0], 1], dtype='f')
        # self.text_feat = np.load(renaming_suffix(self.audio_fn, '.npy'))
        if fm_dict['text_m'] is not None:
            self.text_feat = get_textfeat_bin(self.audio_fn, renaming_suffix(self.audio_fn, '.txt'), fm_dict).astype(np.float32)
        else:
            self.text_feat = np.zeros([full_body.shape[0], 1], dtype='f')
            ...
            def __getitem__(child, index):
                ...
                if not self.context_info:
                    if not self.whole_video:
                        if self.audio_feat.shape[0] > 16000:
                            audio_feat = self.audio_feat[index // 3 * 1600:(index + seq_len) // 3 * 1600]
                        else:
                    ...
                    if self.convert_to_6d:
                        if self.expression:
                            data_sample = {
                                ...
                                'text_feat': text_feat.astype(float).transpose(1, 0),
                            ...
                            data_sample = {
                                ...
                                'text_feat': text_feat.astype(np.float).transpose(1, 0),
                                'speaker': self.speaker,
                                'betas': self.betas
                            }
                    else:
