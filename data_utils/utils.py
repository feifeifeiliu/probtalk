...
def linear_interpolation(features, output_len):
    output_features = F.interpolate(features, size=output_len, mode='linear')
...
def  get_mfcc_ta(audio_fn, fps=15, sr=16000, fm_dict=None, encoder_choice='mfcc'):
    ...
    if encoder_choice == 'faceformer':
        audio_ft = np.squeeze(fm_dict['aud_p'](speech_array, sampling_rate=16000).input_values).reshape(-1, 1)
    elif encoder_choice == 'wav2vec_slice':
        audio_ft = speech_array.reshape(-1, 1)
    elif encoder_choice in ['wav2vec', 'hubert']:
        inputs = fm_dict['aud_p'](audio, sampling_rate=sampling_rate, return_tensors="pt")
        for key in inputs.data:
            inputs.data[key] = inputs.data[key].to('cuda')
        with torch.no_grad():
            outputs = fm_dict['aud_m'](**inputs)
            # outputs = aud_m(**inputs, frame_num=int(speech_array.shape[0]/sampling_rate*30), featex=False)
        audio_ft = outputs.last_hidden_state
        audio_ft = linear_interpolation(audio_ft.transpose(1, 2), int(speech_array.shape[0]/sampling_rate*30))
        audio_ft = audio_ft.squeeze().transpose(0,1).to('cpu').numpy()
