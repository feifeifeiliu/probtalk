...
class Encoder(nn.Module):
    def __init__(self, in_dim, embedding_dim, num_hiddens, num_residual_layers, num_residual_hiddens, ae=False):
...
class VQVAE(nn.Module):
    ...
    def forward(self, gt_poses, id=None, pre_state=None, **kwargs):
        z = self.encoder(gt_poses)
        if not self.training:
            e, _ = self.vq_layer(z)
            x_recon, cur_state = self.decoder(e)
            return e, x_recon

        e, e_q_loss = self.vq_layer(z)
        gt_recon, cur_state = self.decoder(e)
...
class AE(nn.Module):
    ...
    def forward(self, gt_poses, **kwargs):
        z = self.encoder(gt_poses)
        if not self.training:
            x_recon, cur_state = self.decoder(z)
            return z, x_recon
        gt_recon, cur_state = self.decoder(z)
