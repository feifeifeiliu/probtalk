# https://github.com/kakaobrain/rq-vae-transformer/blob/main/rqvae/models/rqvae/quantizations.py
...
from nets.spg.vqvae_modules import QuantizationOutput
...
class RQBottleneck(nn.Module):
    ...
    def __init__(self,
        ...
        embed_dim = np.prod(latent_shape[0]) // np.prod(code_shape[0]) * latent_shape[1]
    ...
    def to_code_shape(self, x):
        (B, H, D) = x.shape
        (rH, _) = self.shape_divisor

        x = x.reshape(B, H // rH, -1)

        return x

    def to_latent_shape(self, x):
        (B, h, _) = x.shape
        (_, D) = self.latent_shape
        (rH, _) = self.shape_divisor
    ...
    def forward(self, x, r, **kwargs):
        ...
        quants_trunc = quants_trunc.transpose(1, 2)

        if not self.training:
            return QuantizationOutput(quant_list[-1].transpose(1, 2), codes.transpose(1, 2))
        return QuantizationOutput(quants_trunc, commitment_loss * 0.25)
