...
class Generator(nn.Module):
    def __init__(self,
        ...
        if self.encoder_choice == 'meshtalk':
            ...
            from data_utils.foundation_models import proxies
            try:
                self.audio_encoder = Wav2Vec2Model.from_pretrained("/home/yifei/.cache/huggingface/hub/wav2vec/wav2vec", proxies=proxies)  # "vitouphy/wav2vec2-xls-r-300m-phoneme""facebook/wav2vec2-base-960h"
            except:
                self.audio_encoder = Wav2Vec2Model.from_pretrained("D:\Downloads\wav2vec",
                                                                   proxies=proxies)
    ...
    def forward(self, in_spec, gt_poses=None, id=None, pre_state=None, time_steps=None):
        ...
        if self.encoder_choice == 'meshtalk':
            ...
            hidden_states = self.audio_encoder(in_spec.transpose(1,2), frame_num=time_steps).last_hidden_state
