...
def tofbhe(kps, c_index):
    if c_index.shape[0] == 129:
        jaw_dim = 3
    else:
        jaw_dim = 6
    jaw = kps[:, :jaw_dim, :]
    exp = kps[:, -exp_dim:, :]
    kps = kps[:, c_index, :]
    kps = torch.cat([jaw, kps, exp], 1)
    return kps

def denormalize(kps, data_mean, data_std, shape='all', c_index=1):
    '''
    kps: (B, T, C)
    '''
    ...
    if shape == 'fbhe':
        # kps = tofbhe(kps, c_index)
        data_std = tofbhe(data_std, c_index)
        data_mean = tofbhe(data_mean, c_index)

    elif shape == 'bh':
        # kps = kps[:, :, c_index]
        data_std = data_std[:, c_index, :]
        data_mean = data_mean[:, c_index, :]
